 ...
 * Description: Unit tests for patch handler.
 */

import { jest } from '@jest/globals';

const patchCustomer = jest.fn<(id: string, p: any) => Promise<any>>();
...
const { handler } = await import('../../../src/handlers/patch.js');
...
describe('patch handler', () => {
  ...
  test('returns updated', async () => {
    patchCustomer.mockResolvedValueOnce({ id: '1' } as any);
    const res = await handler(event);
    expect((res as any).statusCode).toBe(200);
  });

  test('returns 404 when missing', async () => {
    patchCustomer.mockResolvedValueOnce(null as any);
    const res = await handler(event);
